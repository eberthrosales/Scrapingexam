...
from utils.categorizer import categorize_articles
from utils.news_exporter import export_news_to_csv
...
def main():
    ...
    st.markdown("Extrae texto, imágenes, documentos y noticias de cualquier sitio web.")

    # ══════════════════════════════════════════════════════════════════════
    # PESTAÑAS PRINCIPALES
    # ══════════════════════════════════════════════════════════════════════
    tab_scraper, tab_noticias = st.tabs(["🕸️ Scraper General", "📰 Noticias"])

    # ══════════════════════════════════════════════════════════════════════
    # PESTAÑA 1: SCRAPER GENERAL (código existente intacto)
    # ══════════════════════════════════════════════════════════════════════
    with tab_scraper:

        # Sidebar para opciones del Scraper General
        with st.sidebar:
            st.header("⚙️ Scraper General")
            ...
            # ── Opciones de Noticias (siempre visibles, agrupadas) ──
            st.divider()
            st.header("📰 Noticias")
            max_articles = st.slider("Máx. artículos a extraer", 5, 100, 20, key="news_max")
            use_ai_categories = st.checkbox("🤖 Categorizar con IA", value=True, help="Requiere ANTHROPIC_API_KEY configurada como variable de entorno", key="news_ai")
            st.info("Ingresa la URL de una sección de noticias (ej: rpp.pe/politica), no de un artículo individual.")
        ...
        with st.container():
            ...
            with col1:
                url = st.text_input("🔗 Ingrese la URL del sitio web:", placeholder="https://ejemplo.com", help="Pega aquí la URL completa comenzando con http:// o https://", key="scraper_url")
                ...
                try:
                    ...
                    if use_ckan and "datosabiertos.gob.pe" in url:
                        with st.spinner("Extrayendo datasets de datosabiertos.gob.pe ..."):
                            ckan_filters = {}
                            if filter_format:
                                ckan_filters["format"] = filter_format[0]
                            if filter_date:
    ...
    # ══════════════════════════════════════════════════════════════════════
    with tab_noticias:
        st.markdown("### 📰 Extractor de Noticias con Categorización")
        st.markdown("Ingresa la URL de una **sección de noticias** (ej: `rpp.pe/politica`, `elcomercio.pe/economia`). Se extraerán los artículos automáticamente.")

        news_url = st.text_input("🔗 URL de la sección de noticias:", placeholder="https://rpp.pe/politica", key="news_url")

        news_btn = st.button("📰 Extraer Noticias", use_container_width=True, key="news_btn")

        if news_btn:
            if not news_url:
                st.error("Por favor, ingrese la URL de una sección de noticias.")
            elif not validators.url(news_url):
                st.error("La URL no tiene un formato válido.")
            else:
                try:
                    # 1. Detectar tipo de sitio
                    with st.spinner("Detectando estructura del sitio..."):
                        mode = detect_page_type(news_url)
                        st.write(f"✓ **Tipo de página detectado:** `{mode.capitalize()}`")

                    # 2. Extraer artículos
                    with st.spinner(f"Extrayendo lista de artículos (máx. {max_articles})..."):
                        scraper = NewsScraper()
                        articles = scraper.scrape_news_list(news_url, max_articles=max_articles)

                    if not articles:
                        st.warning("No se encontraron artículos de noticias en esta URL. Asegúrate de que sea una sección/listado y no un artículo individual.")
                        st.stop()

                    # 3. Categorizar con IA (si está activo)
                    if use_ai_categories and articles:
                        with st.spinner(f"Categorizando {len(articles)} artículos con IA (lotes de 20)..."):
                            articles = categorize_articles(articles)

                    st.success(f"✓ {len(articles)} artículos extraídos exitosamente.")

                    # ── Métricas ──
                    dominio = urlparse(news_url).netloc
                    categorias_unicas = list(set(a.get("categoria_ia", "Sin clasificar") for a in articles))

                    m1, m2, m3 = st.columns(3)
                    m1.metric("📊 Total artículos", len(articles))
                    m2.metric("📁 Categorías", len(categorias_unicas))
                    m3.metric("🌐 Fuente", dominio)

                    # ── Filtro por categoría ──
                    categorias_sorted = sorted(categorias_unicas)
                    ...
                    # Filtrar artículos si hay filtros activos
                    if categoria_filtro:
                        articles_display = [a for a in articles if a.get("categoria_ia") in categoria_filtro]
                    else:
                        articles_display = articles

                    # ── Visualización por Categorías ──
                    cats_in_display = sorted(set(a.get("categoria_ia", "Sin clasificar") for a in articles_display))

                    for cat in cats_in_display:
                        cat_articles = [a for a in articles_display if a.get("categoria_ia") == cat]
                        with st.expander(f"📁 {cat} ({len(cat_articles)})", expanded=False):
                            # Construir DataFrame para display
                            display_rows = []
                            for art in cat_articles:
                                display_rows.append({
                                    "titulo": art.get("url_fuente", ""),
                                    "Fecha": art.get("fecha", "")[:10] if art.get("fecha") else "",
                                    "Autor": art.get("autor", "Desconocido"),
                                    "Tags": " | ".join(art.get("tags_originales", [])[:5]),
                                    "_display_text": art.get("titulo", "Sin título"),
                                })

                            df_cat = pd.DataFrame(display_rows)

                            st.dataframe(
                                df_cat[["titulo", "Fecha", "Autor", "Tags"]],
                                column_config={
                                    "titulo": st.column_config.LinkColumn(
                                        "Título",
                                        display_text=None,
                                    ),
                                },
                    ...
                    st.download_button(
                        "⬇️ Descargar todas las noticias (CSV)",
                        data=export_news_to_csv(articles),
                        file_name=f"noticias_{dominio}_{fecha_hoy}.csv",
                        mime="text/csv",
                        use_container_width=True,
                        key="news_csv_download"
                    )

                except Exception as e:
                    st.error(f"Error al procesar noticias: {str(e)}")
